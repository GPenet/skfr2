...
struct SBUG {
	...
	int LockedDigit() {
		...
		int iret = 0;// to see if active
		{
			int x = va ^ (1 << dlock), d, uxs = tcellsrcb[ca] & ~unitsab, u1;
		...
		{
			int x = vb ^ (1 << dlock), d, uxs = tcellsrcb[cb] & ~unitsab, u1;
			bitscanforward(u1, uxs);
			while (x) {
				bitscanforward(d, x); x ^= 1 << d;
				if (!((p->dm[d] & units3xBM[u1]).Count96() & 1)) {
